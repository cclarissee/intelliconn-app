/**
 * School of Computer Studies Templates Initialization
 * Provides built-in system templates for school announcements, events, achievements, news, and promotional content
 */
...
export interface SchoolTemplate {
  id: string;
  name: string;
  category: 'Announcements' | 'Events' | 'Achievements' | 'News' | 'Promotional';
  subject: string;
  content: string;
  platform: string[];
...
export const DEFAULT_SCHOOL_TEMPLATES: Omit<SchoolTemplate, 'createdAt'>[] = [
  // ============================================
...
/**
 * Initialize school templates in Firestore
 * This creates a new collection called 'schoolTemplates' with all predefined templates
 */
export async function initializeSchoolTemplates(): Promise<void> {
  try {
    const schoolTemplatesRef = collection(db, 'schoolTemplates');

    // Check if templates already exist
    const existingDocs = await getDocs(schoolTemplatesRef);
    ...
    for (const template of DEFAULT_SCHOOL_TEMPLATES) {
      const docRef = doc(schoolTemplatesRef, template.id);
      await setDoc(docRef, {
        ...template,
        createdAt: new Date(),
      });
      console.log(`[School Templates] Created template: ${template.name}`);
    }
...
/**
 * Get all school templates by category
 */
export async function getSchoolTemplatesByCategory(
  category: string
): Promise<SchoolTemplate[]> {
  try {
    const schoolTemplatesRef = collection(db, 'schoolTemplates');
    const allDocs = await getDocs(schoolTemplatesRef);
    ...
    console.error('[School Templates] Error fetching templates:', error);
    throw error;
  }
}

/**
 * Get all school templates
 */
export async function getAllSchoolTemplates(): Promise<SchoolTemplate[]> {
  try {
    const schoolTemplatesRef = collection(db, 'schoolTemplates');
    const allDocs = await getDocs(schoolTemplatesRef);
    ...
    console.error('[School Templates] Error fetching all templates:', error);
    throw error;
  }
}
